    ...
    : IDataService
{
    ...
    {
        var result = _users.TryRemove(user.Id, out _);
        if (result)
        {
            var filteredTasks = _tasks.Where(value => value.Value.UserId == user.Id);
            foreach (var task in filteredTasks)
            {
    ...
    public Task<bool> RemoveTaskAsync(TaskItem task)
    {
        var result = _tasks.TryRemove(task.Id, out _);
        return Task.FromResult(result);
    }
}